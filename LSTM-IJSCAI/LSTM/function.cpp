...
PVariable FunctionPlus::forward(vector<PVariable > &inputs, vector<PVariable > &outputs){
    ...
    float *tmp_v1; float *tmp_v2; float *tmp_r;
    float *tmp_output;
    
    size_t nBytes_v1 = v1->data.rows * v1->data.cols * sizeof(float);
    size_t nBytes_v2 = v2->data.rows * v2->data.cols * sizeof(float);
    
    size_t nBytes_output = v1->data.rows * v1->data.cols * sizeof(float);

    tmp_v1 = (float*)calloc(nBytes_v1, sizeof(float));
    tmp_v2 = (float*)calloc(nBytes_v2, sizeof(float));
    tmp_output = (float*)calloc(nBytes_output, sizeof(float));

    cublasGetVector(v1->data.rows * v1->data.cols, sizeof(float), v1->data.mDevice, 1, tmp_v1, 1 );
    cublasGetVector(v2->data.rows * v2->data.cols, sizeof(float), v2->data.mDevice, 1, tmp_v2, 1 );
    
    PVariable r = variable_construct_for_function(this, v1->data.rows, v1->data.cols);

    size_t nBytes_r = r->data.rows * r->data.cols * sizeof(float);
    tmp_r = (float*)malloc(nBytes_r);
    
    outputs.push_back(r);
    v1->data.plus(v2->data, r->data);

    cublasGetVector(r->data.rows * r->data.cols, sizeof(float), r->data.mDevice, 1, tmp_r, 1 );

    for(int i = 0; i < nBytes_output; i++)
      tmp_output[i] = tmp_v1[i] + tmp_v2[i];
...
void FunctionPlus::backward(cuMat &p_grad, vector<PVariable > &inputs, vector<PVariable > &outputs){
    ...
    size_t nBytes_v1 = v1->grad.rows * v1->grad.cols * sizeof(float);
    size_t nBytes_v2 = v2->grad.rows * v2->grad.cols * sizeof(float);
    size_t nBytes_p = p_grad.rows * p_grad.cols * sizeof(float);
    size_t nBytes_output = p_grad.rows * p_grad.cols * sizeof(float);
    size_t nBytes_output_v1 = p_grad.rows * p_grad.cols * sizeof(float);
    size_t nBytes_output_v2 = p_grad.rows * p_grad.cols * sizeof(float);
	
    tmp_v1 = (float*)calloc(nBytes_v1, sizeof(float));
    tmp_v2 = (float*)calloc(nBytes_v2, sizeof(float));
    tmp_p = (float*)malloc(nBytes_p);
    tmp_output = (float*)calloc(nBytes_output, sizeof(float));
    tmp_output_v1 = (float*)calloc(nBytes_output_v1, sizeof(float));
    tmp_output_v2 = (float*)calloc(nBytes_output_v2, sizeof(float));
	...
	cublasGetVector(v1->grad.rows * v1->grad.cols, sizeof(float), v1->grad.mDevice, 1, tmp_v1, 1 );
	cublasGetVector(p_grad.rows * p_grad.cols, sizeof(float), p_grad.mDevice, 1, tmp_p, 1 );

	for(int i = 0; i < nBytes_output; i++)
	  {
	    tmp_output[i] = tmp_v1[i] * 1 * tmp_p[i] * 1;
	    tmp_output_v1[i] = tmp_v1[i] * 1 * tmp_p[i] * 1;
	  }
	...
	cublasGetVector(v2->grad.rows * v2->grad.cols, sizeof(float), v2->grad.mDevice, 1, tmp_v2, 1 );
	cublasGetVector(p_grad.rows * p_grad.cols, sizeof(float), p_grad.mDevice, 1, tmp_p, 1 );

	for(int i = 0; i < nBytes_output; i++)
	  {
	    tmp_output[i] = tmp_v2[i] * 1 * tmp_p[i] * 1;
	    tmp_output_v2[i] = tmp_v2[i] * 1 * tmp_p[i] * 1;
	  }
...
PVariable FunctionLinear::forward(vector<PVariable > &inputs, vector<PVariable > &outputs){
    ...
    float *backup;
    int nBytes = x->data.rows * x->data.cols * sizeof(float);

    /*
    backup = (float*)malloc(nBytes); 
    memcpy(backup, x->data.mDevice_host, nBytes); 
    */    
    ...
    float* tmp_w; float* tmp_b; float* tmp_r; float* tmp_x;
    float* tmp_w_2; float* tmp_x_2; float* tmp_r_2;

    size_t nBytes1 = w->data.rows * w->data.cols * sizeof(float);
    size_t nBytes3 = r->data.rows * r->data.cols * sizeof(float);
    size_t nBytes4 = x->data.rows * x->data.cols * sizeof(float);

    tmp_w = (float*)malloc(nBytes1);
    tmp_r = (float*)malloc(nBytes3);
    tmp_x = (float*)malloc(nBytes4);

    tmp_w_2 = (float*)malloc(nBytes1);
    tmp_r_2 = (float*)malloc(nBytes3);
    tmp_x_2 = (float*)malloc(nBytes4); 

    cublasGetVector(w->data.rows * w->data.cols, sizeof(float), w->data.mDevice, 1, tmp_w_2, 1 );
    cublasGetVector(r->data.rows * r->data.cols, sizeof(float), r->data.mDevice, 1, tmp_r_2, 1 );
    cublasGetVector(x->data.rows * x->data.cols, sizeof(float), x->data.mDevice, 1, tmp_x_2, 1 );     

    if (!noBias)
      {
	b->data.dot(i1, r->data);
	size_t nBytes2 = b->data.rows * b->data.cols * sizeof(float);
	tmp_b = (float*)malloc(nBytes2);
	cublasGetVector(b->data.rows * b->data.cols, sizeof(float), b->data.mDevice, 1, tmp_b, 1 );
	free(tmp_b);
      }
	...
	cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
		    w->data.rows, x->data.cols, w->data.cols,
		    alpha, tmp_w_2, w->data.rows,
		    tmp_x_2, x->data.rows,
		    beta, tmp_r_2, r->data.rows);
    ...
    */
    ...
    cublasSetVector(w->data.rows * w->data.cols, sizeof(float), tmp_w_2, 1, w->data.mDevice, 1 );
    cublasSetVector(r->data.rows * r->data.cols, sizeof(float), tmp_r_2, 1, r->data.mDevice, 1 );
    cublasSetVector(x->data.rows * x->data.cols, sizeof(float), tmp_x_2, 1, x->data.mDevice, 1 );    
...
PVariable FunctionSigmoid::forward(vector<PVariable > &inputs, vector<PVariable > &outputs){
    ...
    float* tmp_x; 
    size_t nBytes_x = x->data.rows * x->data.cols * sizeof(float);
    
    tmp_x = (float*)malloc(nBytes_x);
    ...
    float* tmp_output; 
    size_t nBytes_output = r->data.rows * r->data.cols * sizeof(float);
    tmp_output = (float*)calloc(nBytes_output, sizeof(float));
    ...
    for(int i = 0; i < nBytes_output; i++)
      tmp_output[i] = 1.0f/(1.0f + std::exp(-tmp_x[i]));
...
PVariable FunctionMeanSquaredError::forward(vector<PVariable > &inputs, vector<PVariable > &outputs){
    ...
    float *tmp_x; float *tmp_t; float *tmp_r;
    float *tmp_output;
    
    size_t nBytes_x = x->data.rows * x->data.cols * sizeof(float);
    size_t nBytes_t = t->data.rows * t->data.cols * sizeof(float);
    
    size_t nBytes_output = x->data.rows * x->data.cols * sizeof(float);

    tmp_x = (float*)calloc(nBytes_x, sizeof(float));
    tmp_t = (float*)calloc(nBytes_t, sizeof(float));
    tmp_output = (float*)calloc(nBytes_output, sizeof(float));
